...
while ($reader->nextElement("A2A", "http://Mindbus.nl/A2A")) {
    ...
    if( my $docnr = $a2a->{'Source'}->{SourceReference}->{DocumentNumber}->{value} ) {
        ...
        if( $docnr eq '') {
            $logs->write_row($err, 0, &logErr('BS_G','AKTENUMMER_LEEG','DocumentNumber', "", 
        ...
        # REGEL 2: het aktenummer bestaat alleen uit getallen gevolgd door een kleine letter of een hoofdletter S (van supplement)
            $logs->write_row($err, 0, &logErr('BS_G','AKTENUMMER_ONBEKEND','DocumentNumber', $docnr, 
    ...
    if( ref($root->{A2A}->{Person}) eq 'ARRAY' ) {
        ...
        $logs->write_row($err, 0, &logErr('BS_G','GEEN_PERSONEN','','',"De akte bevat geen personen", $a2a, "AKTE"));
    }
    ...
    if( defined $persons and scalar @{$persons} > 3 ) {
        $logs->write_row($err, 0, &logErr('BS_G','TEVEEL_PERSONEN','AantalPersonen', scalar @{$persons}, 
    ...
    foreach my $p (@{$persons}) {
        ...
        unless($rol = $relmap{$p->{pid}->{value}}) {
            ...
            $logs->write_row($err, 0, &logErr('BS_G',"GEEN_ROL","RelationType", "", 
        ...
        if( $rol eq 'Relatie' ) {
            ...
            $logs->write_row($err, 0, &logErr('BS_G',"DUBBELE_ROL","RelationType", $rol, 
        ...
        if( not(length($p->{PersonName}->{PersonNameLastName}->{value})) 
            ...
            # 
            $logs->write_row($err, 0, &logErr('BS_G',"NAAMDEEL_LEEG",'PatroniemOfAchternaam', "", 
            ...
            $logs->write_row($err, 0, &logErr('BS_G',"NAAMDEEL_LEEG",'Voornaam', "", 
        ...
        foreach my $w (qw/PersonNameLastName PersonNameFirstName PersonNamePatronym/) {
            if( defined($p->{PersonName}->{$w}->{value}) ) {
                if( lc $p->{PersonName}->{$w}->{value} !~ $nnescio and $p->{PersonName}->{$w}->{value} ne "-") {
                        ...
                        $logs->write_row($err, 0, &logErr('BS_G',"WAARDE_VERDACHT",$w, $p->{PersonName}->{$w}->{value}, 
                ...
                if( length $p->{PersonName}->{$w}->{value} && $p->{PersonName}->{$w}->{value} =~ qr/[aeiou]{$alg->{'max_klinkers'},}/ ) {
                    $logs->write_row($err, 0, &logErr('BS_G',"KLINKERS", $w, $p->{PersonName}->{$w}->{value},
                ...
                if( length $p->{PersonName}->{$w}->{value} && $p->{PersonName}->{$w}->{value} =~ qr/[bcdfghklmnpqrstvwx]{$alg->{'max_medeklinkers'},}/ ) {
                    $logs->write_row($err, 0, &logErr('BS_G',"MEDEKLINKERS", $w, $p->{PersonName}->{$w}->{value},
                ...
                if( length $p->{PersonName}->{$w}->{value} && $p->{PersonName}->{$w}->{value} =~ /([^\.])\1\1/ ) {
                    $logs->write_row($err, 0, &logErr('BS_G',"HERHALING", $w, $p->{PersonName}->{$w}->{value},
        ...
        if( my $fn = $p->{PersonName}->{PersonNameFirstName}->{value} ) {
             if( my $gr = $p->{Gender}->{value} ) {
                if( $gr eq "Man" or $gr eq "Vrouw" ) {
                  ...
                  $logs->write_row( $err, 0, &logErr('BS_G',"GESLACHT_FOUT", "PersonNameFirstName", $temp[0],
        ...
        if( $p->{PersonName}->{PersonNamePrefixLastName}->{value} ) {
            unless( $p->{PersonName}->{PersonNamePrefixLastName}->{value} =~ $re_tv ) {
                ...
                $logs->write_row($err, 0, &logErr('BS_G',"WAARDE_VERDACHT",'PersonNamePrefixLastName', $p->{PersonName}->{PersonNamePrefixLastName}->{value}, 
        ...
        if( $p->{PersonName}->{PersonNamePatronym}->{value} ) {
            ...
            unless( $p->{PersonName}->{PersonNamePatronym}->{value} =~ $re_pn ) {
                ...
                $logs->write_row($err, 0, &logErr('BS_G',"WAARDE_VERDACHT",'PersonNamePatronym', $p->{PersonName}->{PersonNamePatronym}->{value}, 
        ...
        if( my $prof = $p->{Profession}->{value} ) {
            unless( $prof eq "" or $prof =~ /^([\p{Ll}\p{Lu}0-9\'\-\(\)\.\/ ,]|&amp;)+$/ ) {
                ...
                $logs->write_row($err, 0, &logErr('BS_G',"WAARDE_VERDACHT",'Profession', $prof, 
        ...
        if( my $pal = $p->{Age}->{PersonAgeLiteral}->{value} ) {
            if( $pal =~ /^(\d+)( (jaar|jaren))?$/ ) {
                ...
                if( $age < $bsg->{min_leeftijd} or $age > $bsg->{max_leeftijd} ) {
                    ...
                    $logs->write_row($err, 0, &logErr('BS_G',"LEEFTIJD",'PersonAgeLiteral', $age, 
        ...
        if( my $bplace = $p->{BirthPlace}->{Place}->{value} ) {
            ...
            unless( $bplace =~ /^([\p{Ll}\p{Lu}\'\-\., \(\)\/\[\]]|&amp;)+$/ ) {
                $logs->write_row($err, 0, &logErr('BS_G',"WAARDE_VERDACHT",'BirthPlace', $bplace, 
    ...
    if( my $yr = $a2a->{Source}->{SourceDate}->{Year}->{value} ) {
        if( $yr < $bsg->{min_jaar} or $yr > $bsg->{max_jaar} ) {
            ...
            $logs->write_row($err, 0, &logErr('BS_G',"DATUM_FOUT",'SourceDate', $yr,
        ...
        if( my $mnd = $a2a->{Source}->{SourceDate}->{Month}->{value} and my $dag = $a2a->{Source}->{SourceDate}->{Day}->{value} ) {
            ...
            #REGEL 24: de datum moet geldig zijn
                $logs->write_row($err, 0, &logErr('BS_G',"DATUM_FOUT",'EventDate', $yr."-".$mnd."-".$dag,
    ...
    if( defined $remark and $remark->{Value}->{value} eq $bsg->{'levenloos'} ) {
        ...
        # die heeft dan geen relaties
            ...
            $logs->write_row($err, 0, &logErr('BS_G',"SOORT_FOUT",'SourceRemark', 'AkteSoort',
                    "Deze levenloos geboren persoon heeft een relatie. Klopt dit wel?", $a2a, "PERSOON: ".&maakNaam($fam{'Kind'})." (Kind)"));
        }
    }
}
